...
_apply_bindings() {
  ...
  perl -p -i -e "
    s,\bnew-window\b((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!\bssh\b)[^\s]+))*)?(?:\s+(\bssh\b))((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!\bssh\b)[^\s]+))*)?,run-shell 'sh \"$OH_MY_TMUX\" _new_window_ssh #\{pane_pid\} #\{b:pane_tty\}\1',g if /\bnew-window\b((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!ssh)[^\s]+))*)?(?:\s+(ssh))((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!ssh)[^\s]+))*)?/" \
    "$cfg"
  ...
  if ! _is_disabled "$tmux_conf_new_window_reconnect_ssh" && _is_true "$tmux_conf_new_window_reconnect_ssh"; then
    perl -p -i -e "s,\bnew-window\b([^;}\n\"]*),run-shell 'sh \"$OH_MY_TMUX\" _new_window #\{pane_pid\} #\{b:pane_tty\}\1',g" "$cfg"
  fi
  ...
  perl -p -i -e "
    s,\bsplit-window\b((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!\bssh\b)[^\s]+))*)?(?:\s+(\bssh\b))((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!\bssh\b)[^\s]+))*)?,run-shell 'sh \"$OH_MY_TMUX\" _split_window_ssh #\{pane_pid\} #\{b:pane_tty\}\1',g if /\bsplit-window\b((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!ssh)[^\s]+))*)?(?:\s+(ssh))((?:(?:[ \t]+-[bdfhIvP])|(?:[ \t]+-[celtF][ \t]+(?!ssh)[^\s]+))*)?/" \
    "$cfg"
  ...
  if ! _is_disabled "$tmux_conf_new_pane_reconnect_ssh" && _is_true "$tmux_conf_new_pane_reconnect_ssh"; then
    perl -p -i -e "s,\bsplit-window\b([^;}\n\"]*),run-shell 'sh \"$OH_MY_TMUX\" _split_window #\{pane_pid\} #\{b:pane_tty\}\1',g" "$cfg"
  fi
...
_apply_theme() {
  ...
  *'#{uptime_'* | *'#{?uptime_'*)
  ...
  _pkillf "sh '$OH_MY_TMUX' _loadavg"
  case "$status_left $status_right" in
  *'#{loadavg'* | *'#{?loadavg'*)
    ...
    interval=$(tmux show -gv status-interval)
    if [ "$_tmux_version" -ge 320 ]; then
      tmux run -b "trap '[ -n \"\$sleep_pid\" ] && kill -9 \"\$sleep_pid\"; exit 0' TERM; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice sh '$OH_MY_TMUX' _loadavg; sleep $interval & sleep_pid=\$!; wait \"\$sleep_pid\"; sleep_pid=; done"
    elif [ "$_tmux_version" -ge 280 ]; then
      status_right="#(echo; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice sh '$OH_MY_TMUX' _loadavg; sleep $interval; done)$status_right"
    elif [ "$_tmux_version" -gt 240 ]; then
      status_right="#(echo; while :; do nice sh '$OH_MY_TMUX' _loadavg; sleep $interval; done)$status_right"
    else
      status_right="#(nice sh '$OH_MY_TMUX' _loadavg)$status_right"
    fi
    ;;
  esac
  ...
  tmux set -g set-titles-string "$(_decode_unicode_escapes "$set_titles_string")" \; setw -g window-status-format "$(_decode_unicode_escapes "$window_status_format")" \; setw -g window-status-current-format "$(_decode_unicode_escapes "$window_status_current_format")" \; set -g status-left-length 1000 \; set -g status-left "$(_decode_unicode_escapes "$status_left")" \; set -g status-right-length 1000 \; set -g status-right "$(_decode_unicode_escapes "$status_right")"
}
