...
function M.cmp_source(name, icon)
  ...
  local colors = {
    ok = LoongVim.ui.fg("Special"),
    error = LoongVim.ui.fg("DiagnosticError"),
    pending = LoongVim.ui.fg("DiagnosticWarn"),
  }

  return {
    function()
      return icon or LoongVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
    end,
    cond = function()
      return status() ~= nil
    end,
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = LoongVim.root.get({ normalize = true })
    local cwd = LoongVim.root.cwd()
