...
function M.save()
  LoongVim.config.json.data.version = LoongVim.config.json.version
  local f = io.open(LoongVim.config.json.path, "w")
  if f then
    f:write(LoongVim.json.encode(LoongVim.config.json.data))
    f:close()
  end
end
