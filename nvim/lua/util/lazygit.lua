...
-- Opens lazygit
---@param opts? LoongTermOpts | {args?: string[]}
function M.open(opts)
  ...
  if vim.g.lazygit_config then
    ...
    if not M.config_dir then
      ...
      if ok then
        ...
        local custom_config = LazyUtil.norm(M.config_dir .. "/custom.yml")
  ...
  return LoongVim.terminal(cmd, opts)
end
...
function M.get_color(v)
  ...
  local color = {}
  if v.fg then
    color[1] = LoongVim.ui.color(v.fg)
  elseif v.bg then
    color[1] = LoongVim.ui.color(v.bg, true)
  end
  if v.bold then
...
---@param opts? {count?: number}|LoongCmdOptions
function M.blame_line(opts)
  ...
  local root = LoongVim.root.detectors.pattern(0, { ".git" })[1] or "."
...
function M.browse()
  ...
  local function open(remote)
    if remote then
      LoongVim.info(("Opening [%s](%s)"):format(remote.name, remote.url))
  ...
  if #remotes == 0 then
    return LoongVim.error("No git remotes found")
