...
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        LoongVim.info("Enabled " .. toggle.name, { title = toggle.name })
      else
        LoongVim.warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
...
function M.wk(lhs, toggle)
  if not LoongVim.has("which-key.nvim") then
    return
  end
  local function safe_get()
    local ok, enabled = pcall(toggle.get)
    if not ok then
      LoongVim.error({ "Failed to get toggle state for **" .. toggle.name .. "**:\n", enabled }, { once = true })
    end
    return enabled
  end
...
function M.format(buf)
  return M.wrap({
    ...
    get = function()
      ...
      return LoongVim.format.enabled()
    end,
    set = function(state)
      LoongVim.format.enable(state, buf)
    end,
  })
end
  ...
  set = function(state)
    if state then
      ...
      vim.api.nvim_create_autocmd("ExitPre", {
        once = true,
        group = vim.api.nvim_create_augroup("loongvim_restore_max_exit_pre", { clear = true }),
