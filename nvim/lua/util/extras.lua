...
---@class loongvim.util.extras
local M = {}
M.buf = 0

---@type LoongExtraSource[]
M.sources = {
  { name = "LoongVim", desc = "LoongVim extras", module = "loongvim.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("loongvim.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #LoongVim.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get_extra(source, modname)
  ...
  return {
    ...
    managed = vim.tbl_contains(LoongVim.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end

---@class LoongExtraView
---@field float LazyFloat
---@field text Text
---@field extras LoongExtra[]
---@field diag LazyDiagnostic[]
local X = {}

---@return LoongExtraView
function X.new()
  local self = setmetatable({}, { __index = X })
  M.buf = vim.api.nvim_get_current_buf()
  self.float = Float.new({ title = "LoongVim Extras" })
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      if not extra.managed then
        LoongVim.error(
          "Not managed by LoongExtras. Remove from your config to enable/disable here.",
          { title = "LoongExtras" }
        )
        return
      end
      extra.enabled = not extra.enabled
      LoongVim.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, LoongVim.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(LoongVim.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(LoongVim.config.json.data.extras)
      LoongVim.json.save()
      LoongVim.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart LoongVim to apply the changes.",
        { title = "LoongExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("LoongVim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled LoongVim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  if not extra.managed then
    ...
    elseif vim.tbl_contains(LoongVim.plugin.core_imports, extra.module) then
      self:diagnostic({
        message = "This extra is included by default",
      })
    else
      self:diagnostic({
        message = "Not managed by LoongExtras (config)",
  ...
  if extra.source.name ~= "LoongVim" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^loongvim.plugins.extras.", "")
