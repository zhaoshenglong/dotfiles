...
function M.rename_file()
  ...
  local old = assert(LoongVim.root.realpath(vim.api.nvim_buf_get_name(buf)))
  local root = assert(LoongVim.root.realpath(LoongVim.root.get({ normalize = true })))
...
---@param opts? LoongFormatter| {filter?: (string|lsp.Client.filter)}
function M.formatter(opts)
  ...
  return LazyUtil.merge(ret, opts) --[[@as LoongFormatter]]
end
...
function M.format(opts)
  opts = vim.tbl_deep_extend(
    ...
    {},
    opts or {},
    LoongVim.opts("nvim-lspconfig").format or {},
    LoongVim.opts("conform.nvim").format or {}
  )
  ...
  M.on_supports_method("textDocument/documentHighlight", function(_, buf)
    vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
      ...
      callback = function(ev)
        if not require("loongvim.plugins.lsp.keymaps").has(buf, "documentHighlight") then
          return false
        end
        ...
        if not ({ M.words.get() })[2] then
          if ev.event:find("CursorMoved") then
            vim.lsp.buf.clear_references()
          elseif not LoongVim.cmp.visible() then
